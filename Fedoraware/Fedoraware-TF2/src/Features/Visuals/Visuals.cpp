...
void CVisuals::ScopeLines(CBaseEntity* pLocal)
{
	if (pLocal->IsScoped() && Vars::Visuals::RemoveScope.Value && Vars::Visuals::ScopeLines.Value)
	{
		...
		const Color_t line1 = {Vars::Colors::NoscopeLines1.Value.r, Vars::Colors::NoscopeLines1.Value.g, Vars::Colors::NoscopeLines1.Value.b, 255};
		const Color_t line2 = {Vars::Colors::NoscopeLines2.Value.r, Vars::Colors::NoscopeLines2.Value.g, Vars::Colors::NoscopeLines2.Value.b, 255};
	...
	if (Vars::Misc::CL_Move::Enabled.Value && Vars::Misc::CL_Move::DTBarStyle.Value)
	{
		...
		if (pWeapon)
		{
			if (pLocal->GetLifeState() == LIFE_ALIVE)
			{
				...
				if (G::WaitForShift)
				{
					color1 = Vars::Colors::DTBarIndicatorsCharging.Value.startColour;
					color2 = Vars::Colors::DTBarIndicatorsCharging.Value.endColour;
				}
				else
				{
					color1 = Vars::Colors::DTBarIndicatorsCharged.Value.startColour;
					color2 = Vars::Colors::DTBarIndicatorsCharged.Value.endColour;
				}
				...
				case 1:
					{
						g_Draw.OutlinedRect(DTBox.x, DTBox.y, DTBox.w, DTBox.h, Vars::Colors::DtOutline.Value);
				...
				case 2:
					{
						...
						g_Draw.RoundedBoxStatic(DTBox.x, DTBox.y, DTBox.w, DTBox.h, 4, Vars::Colors::DtOutline.Value);
				...
				case 3:
					{
						g_Draw.OutlinedRect(DTBox.x, DTBox.y, DTBox.w, DTBox.h, Vars::Colors::DtOutline.Value); //	draw the outline
...
bool ModColChanged() // check if colours have been changed
{
	...
	if (Vars::Colors::WorldModulation.Value != oldW || Vars::Colors::SkyModulation.Value != oldS)
	{
		oldW = Vars::Colors::WorldModulation.Value;
		oldS = Vars::Colors::SkyModulation.Value;
...
void CVisuals::ModulateWorld()
{
	...
	{
		Vars::Visuals::WorldModulation.Value ? ApplyModulation(Vars::Colors::WorldModulation.Value) : ApplyModulation({255, 255, 255, 255});
		Vars::Visuals::SkyModulation.Value ? ApplySkyboxModulation(Vars::Colors::SkyModulation.Value) : ApplySkyboxModulation({255, 255, 255, 255});
